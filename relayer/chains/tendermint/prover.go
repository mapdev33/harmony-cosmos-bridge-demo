...
func (pr *Prover) CreateMsgCreateClient(clientID string, dstHeader core.HeaderI, signer sdk.AccAddress) (*clienttypes.MsgCreateClient, error) {
	...
	tmHeader, ok := dstHeader.(*types.TmHeader)
	if !ok {
		return nil, errors.New("dstHeader is not TmHeader")
	}
...
func (pr *Prover) UpdateLightWithHeader() (header core.HeaderI, provableHeight int64, queryableHeight int64, err error) {
	...
	h := &types.TmHeader{
		SignedHeader: types.NewSignedHeaderFromTm(sh.SignedHeader),
		ValidatorSet: types.NewValidatorSetFromTm(tmtypes.NewValidatorSet(sh.ValidatorSet.Validators)),
	}
...
func (c *Prover) queryHeaderAtHeight(height int64) (*types.TmHeader, error) {
	var (
		page    int = 1
		perPage int = 100000
	)
	...
	return &types.TmHeader{
		// NOTE: This is not a SignedHeader
		// We are missing a light.Commit type here
		SignedHeader: types.NewSignedHeaderFromTm(&res.SignedHeader),
		ValidatorSet: types.NewValidatorSetFromTm(tmtypes.NewValidatorSet(val.Validators)),
	}, nil
}
